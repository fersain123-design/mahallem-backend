...
exports.getLocationStats = exports.resolveLocation = void 0;
...
const resolveLocation = async (req, res) => {
    try {
        ...
        const isTurkeyArea = numericLat >= 34.0 && numericLat <= 44.0 &&
        ...
        let result = null;
        try {
            const polygonMatch = (0, neighborhoodPolygonService_1.findNeighborhoodByCoordinate)(numericLat, numericLng);
        ...
        if (!result || !result.mahalle) {
            try {
                const nominatimResult = await (0, geocodeService_1.reverseGeocodeNominatim)(numericLat, numericLng);
                if (nominatimResult) {
                    result = {
        ...
        return res.status(200).json({
            ...
            meta: {
                ...
                polygonCount: (0, neighborhoodPolygonService_1.getPolygonCount)(),
            },
        });
    }
    catch (error) {
 ...
 */
const getLocationStats = (_req, res) => {
    try {
        const stats = (0, geocodeService_1.getGeocodeStats)();
        return res.json({
            success: true,
            data: {
                ...stats,
