...
function pointInRing(lat, lng, ring) {
    ...
    for (let i = 0, j = n - 1; i < n; j = i++) {
        ...
        if (pointOnSegment(lat, lng, [xi, yi], [xj, yj]))
            return true;
...
function pointInPolygon(lat, lng, coords) {
    ...
    for (let i = 1; i < coords.length; i++) {
        if (pointInRing(lat, lng, coords[i]))
            return false;
    }
    return true;
}
...
function computeBBoxFromOuterRing(ring) {
...
function pickInteriorPoint(g) {
    const outerRing = g.type === 'Polygon' ? g.coordinates?.[0] : g.coordinates?.[0]?.[0];
    if (!outerRing || outerRing.length < 4)
        return null;
    const [minX, minY, maxX, maxY] = computeBBoxFromOuterRing(outerRing);
    const center = { lat: (minY + maxY) / 2, lng: (minX + maxX) / 2 };
    if (pointInGeometry(center.lat, center.lng, g))
        return center;
...
async function main() {
    ...
    const fc = (0, neighborhoodPolygonService_1.getAllNeighborhoodPolygons)();
    ...
    for (const f of features) {
        ...
        if (!p)
            continue;
        const match = (0, neighborhoodPolygonService_1.findNeighborhoodByCoordinate)(p.lat, p.lng);
        ...
        if (got === expected) {
            ...
            if (failures.length < 10)
                failures.push({ expected, got, lat: p.lat, lng: p.lng });
        }
    }
