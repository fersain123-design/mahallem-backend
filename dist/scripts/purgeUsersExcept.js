...
async function getEnvOrPrompt(name, prompt) {
...
async function getEnvOrPromptHidden(name, prompt) {
    const value = String(process.env[name] || '').trim();
    if (value)
        return value;
...
function assertSafeToRun() {
    ...
    const looksLocal = dbUrl.startsWith('file:') ||
    ...
    if (!looksLocal) {
        throw new Error('Refusing to run: DATABASE_URL does not look like a local database. ' +
...
async function ensureAdminUser() {
    ...
    const existing = await db_1.default.user.findUnique({ where: { email }, select: { id: true, email: true } });
    ...
    const passwordHash = password ? await (0, passwordUtils_1.hashPassword)(password) : undefined;
    if (!existing) {
        if (!passwordHash) {
...
async function main() {
    ...
    if (!confirmed) {
        const typed = (await promptLine('This will DELETE ALL USERS except the admin AND DELETE ALL PRODUCTS + ORDERS. Type YES to continue: '))
            .trim()
            .toUpperCase();
        confirmed = typed === 'YES';
    }
    ...
    const usersToDelete = await db_1.default.user.findMany({
