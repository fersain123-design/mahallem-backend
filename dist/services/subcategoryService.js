...
exports.resolveVendorScopedCategoryMeta = exports.ensureBaseCategorySystem = exports.resolveCategoryIdForBusinessType = exports.ensureVendorPrimaryCategory = exports.STORE_TYPE_TO_CATEGORY_SLUG = exports.BASE_CATEGORY_SEED = void 0;
const db_1 = __importDefault(require("../config/db"));
const errorHandler_1 = require("../middleware/errorHandler");
const storeCategories_1 = require("../config/storeCategories");
const slugifyTr = (input) => String(input || '')
    .trim()
    ...
    .replace(/^-+|-+$/g, '')
    .slice(0, 80);
const MARKET_MANAV_SUBCATEGORY_SEED = [
...
const PASTANE_SUBCATEGORY_SEED = [
...
const FIRIN_PASTANE_SUBCATEGORY_SEED = [
    ...FIRIN_SUBCATEGORY_SEED,
    ...PASTANE_SUBCATEGORY_SEED,
];
const KASAP_SUBCATEGORY_SEED = [
...
const SARKUTERI_SUBCATEGORY_SEED = [
...
const KASAP_SARKUTERI_SUBCATEGORY_SEED = [
    ...KASAP_SUBCATEGORY_SEED,
    ...SARKUTERI_SUBCATEGORY_SEED,
];
exports.BASE_CATEGORY_SEED = [
    {
        name: 'Kasap',
        slug: 'kasap',
...
exports.STORE_TYPE_TO_CATEGORY_SLUG = {
    market: 'market',
    manav: 'manav',
...
const mapBusinessTypeToCategorySlug = (businessType) => {
    ...
    if (hasMarket && hasManav)
        return 'market-manav';
    if (hasKasap && hasSarkuteri)
    ...
    if (hasSarkuteri)
        return 'sarkuteri';
    if (text.includes('balik'))
        return 'balikci';
    if (text.includes('kuruyemis'))
        return 'kuruyemisci';
    if (text.includes('aktar'))
        return 'aktar';
    if (text.includes('cicek'))
        return 'cicekci';
    if (text.includes('pet'))
        return 'petshop';
    if (text.includes('bufe'))
        return 'bufe';
    const normalizedStoreType = (0, storeCategories_1.normalizeStoreType)(businessType);
    return exports.STORE_TYPE_TO_CATEGORY_SLUG[normalizedStoreType] || 'market';
};
const buildVendorCategorySlug = (vendorId, categorySlug) => `vendor-${vendorId}-${String(categorySlug || 'category').trim()}`;
const isSeedBackedVendorCategory = (category) => {
    const normalizedName = slugifyTr(String(category?.name || ''));
    const slug = String(category?.slug || '').trim();
    return exports.BASE_CATEGORY_SEED.some((seed) => normalizedName === slugifyTr(seed.name) || slug.endsWith(`-${seed.slug}`));
};
const syncCategorySubCategories = async (categoryId, subCategories) => {
    const desiredRows = [...subCategories];
    for (const sub of desiredRows) {
        await db_1.default.subCategory.upsert({
            where: {
                categoryId_slug: {
                    categoryId,
                    slug: sub.slug,
    ...
    await db_1.default.subCategory.updateMany({
        where: {
            categoryId,
...
const ensureVendorPrimaryCategory = async (vendor) => {
    await (0, exports.ensureBaseCategorySystem)();
    const expectedSlug = mapBusinessTypeToCategorySlug(vendor.businessType);
    const categorySeed = exports.BASE_CATEGORY_SEED.find((item) => item.slug === expectedSlug) ||
        exports.BASE_CATEGORY_SEED.find((item) => item.slug === 'market') ||
        exports.BASE_CATEGORY_SEED[0];
    if (!categorySeed) {
        throw new errorHandler_1.AppError(400, 'Varsayilan kategori bulunamadi');
    }
    ...
    if (!primary) {
        primary = vendorCategories.find((item) => slugifyTr(String(item.name || '')) === slugifyTr(categorySeed.name));
    }
    if (!primary) {
    ...
    if (!primary) {
        ...
        primary = await db_1.default.category.create({
            data: {
                vendorId: vendor.id,
                storeType: (0, storeCategories_1.normalizeStoreType)(vendor.businessType),
    ...
    const expectedStoreType = (0, storeCategories_1.normalizeStoreType)(vendor.businessType);
    if (primary &&
        ...
        primary = await db_1.default.category.update({
            where: { id: primary.id },
            data: {
                name: categorySeed.name,
...
exports.ensureVendorPrimaryCategory = ensureVendorPrimaryCategory;
const resolveCategoryIdForBusinessType = async (businessType) => {
    await (0, exports.ensureBaseCategorySystem)();
...
exports.resolveCategoryIdForBusinessType = resolveCategoryIdForBusinessType;
const ensureBaseCategorySystem = async () => {
    const desiredBaseSlugs = exports.BASE_CATEGORY_SEED.map((category) => category.slug);
    for (const categorySeed of exports.BASE_CATEGORY_SEED) {
    ...
    const removedBaseCategories = await db_1.default.category.findMany({
        where: {
            vendorId: null,
            isCustom: false,
            isActive: true,
    ...
    if (removedBaseCategories.length > 0) {
        ...
        await db_1.default.category.updateMany({
            where: {
...
exports.ensureBaseCategorySystem = ensureBaseCategorySystem;
const findVendorCategory = async (vendor) => {
    const primary = await (0, exports.ensureVendorPrimaryCategory)(vendor);
    if (!primary) {
        throw new errorHandler_1.AppError(400, 'Vendor ana kategorisi bulunamadi');
    }
    return primary;
};
const findSubCategoryByKeyword = async (categorySlug, categoryId, productName) => {
    ...
    if (!matchedSlug)
        return null;
    const subCategory = await db_1.default.subCategory.findFirst({
...
const resolveVendorScopedCategoryMeta = async (vendor, data, required) => {
    ...
    if (rawCategoryId) {
        ...
        if (!category) {
            throw new errorHandler_1.AppError(400, 'Secilen kategori saticiya ait degil');
        }
    }
    ...
    if (rawSubCategoryId) {
        ...
        if (!subCategory) {
            throw new errorHandler_1.AppError(400, 'Secilen alt kategori saticinin ana kategorisine uygun degil');
        }
        return { category, subCategory };
    }
    if (rawSubCategoryName) {
        const candidates = await db_1.default.subCategory.findMany({
            where: {
                categoryId: category.id,
                isActive: true,
            },
        ...
        const subCategory = candidates.find((x) => slugifyTr(String(x.name || '')) === slugifyTr(rawSubCategoryName));
        if (subCategory) {
            return { category, subCategory };
        }
    }
...
exports.resolveVendorScopedCategoryMeta = resolveVendorScopedCategoryMeta;
