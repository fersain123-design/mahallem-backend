...
exports.processQueuedProductImage = void 0;
...
const PRODUCT_IMAGE_WHITE_ITEM_PROTECT_ENABLED = String(process.env.PRODUCT_IMAGE_WHITE_ITEM_PROTECT_ENABLED || '1') !== '0';
const PRODUCT_IMAGE_WHITE_ITEM_SHADOW_ENABLED = String(process.env.PRODUCT_IMAGE_WHITE_ITEM_SHADOW_ENABLED || '1') !== '0';
const PRODUCT_IMAGE_WHITE_ITEM_MEAN_MIN = Math.min(255, Math.max(150, Number(process.env.PRODUCT_IMAGE_WHITE_ITEM_MEAN_MIN || 192)));
const PRODUCT_IMAGE_WHITE_ITEM_SATURATION_MAX = Math.min(40, Math.max(2, Number(process.env.PRODUCT_IMAGE_WHITE_ITEM_SATURATION_MAX || 9)));
const PRODUCT_IMAGE_WHITE_ITEM_SHADOW_BASE_ALPHA = Math.min(0.35, Math.max(0.04, Number(process.env.PRODUCT_IMAGE_WHITE_ITEM_SHADOW_BASE_ALPHA || 0.16)));
const PRODUCT_IMAGE_WHITE_ITEM_SHADOW_BLUR_SIGMA = Math.min(12, Math.max(0.5, Number(process.env.PRODUCT_IMAGE_WHITE_ITEM_SHADOW_BLUR_SIGMA || 4.2)));
...
const IMAGE_CLEANER_ENSURE_SCRIPT = process.env.IMAGE_CLEANER_ENSURE_SCRIPT?.trim() ||
    path_1.default.join(BACKEND_ROOT, 'scripts', 'ensure_image_cleaner.js');
...
const getSharpModule = async () => {
    if (!sharpModulePromise) {
        sharpModulePromise = Promise.resolve().then(() => __importStar(require('sharp'))).then((mod) => mod?.default || mod)
            .catch((error) => {
            console.warn('[product-image] sharp module unavailable:', String(error?.message || error));
            return null;
        });
    }
...
const smartEnhanceProductImage = async (inputBuffer) => {
    ...
    const applySubtleWhiteProductShadow = async (imageBuffer, canvasWidth, canvasHeight, subjectWidth, subjectHeight, fillRatio) => {
        ...
        const dynamicAlpha = Math.min(0.24, Math.max(0.08, PRODUCT_IMAGE_WHITE_ITEM_SHADOW_BASE_ALPHA + (0.16 - Math.min(fillRatio, 0.16)) * 0.22));
        const shadowSvg = Buffer.from(`<svg width="${safeW}" height="${safeH}" xmlns="http://www.w3.org/2000/svg">
    ...
    const isWhiteDominantSubject = async (imageBuffer, estimatedWidth, estimatedHeight) => {
        if (!PRODUCT_IMAGE_WHITE_ITEM_PROTECT_ENABLED) {
            return false;
        }
        try {
            const stats = await sharp(imageBuffer, { failOn: 'none', limitInputPixels: 40000000 }).stats();
    ...
    let finalBuffer = await enhanced
        ...
        .toBuffer();
    if (whiteDominantSubject) {
        const finalMeta = await sharp(finalBuffer, { failOn: 'none', limitInputPixels: 40000000 }).metadata();
        finalBuffer = await applySubtleWhiteProductShadow(finalBuffer, Number(finalMeta?.width || PRODUCT_IMAGE_MIN_WIDTH), Number(finalMeta?.height || PRODUCT_IMAGE_MIN_HEIGHT), Math.max(1, Math.floor(trimWidth)), Math.max(1, Math.floor(trimHeight)), fillRatio);
    }
    return finalBuffer;
};
...
const readJobBuffer = async (input) => {
    if (input.kind === 'file') {
    ...
    if (url.startsWith('/uploads/')) {
        ...
        const absPath = path_1.default.join(process.cwd(), relative);
        const fileBuffer = fs_1.default.readFileSync(absPath);
        const filename = sanitizeFilename(path_1.default.basename(absPath));
        return {
            buffer: fileBuffer,
            filename,
    ...
    const filename = sanitizeFilename(path_1.default.basename(parsedUrl.pathname) || `image_${Date.now()}.png`);
    const mimeType = normalizeImageMimeType(response.headers.get('content-type') || undefined, filename, buffer);
    return { buffer, filename, mimeType };
};
const persistProcessedImage = (buffer, suggestedName, filePrefix) => {
...
const processQueuedProductImage = async (input, filePrefix) => {
        ...
        if (dimensions.width < PRODUCT_IMAGE_MIN_WIDTH || dimensions.height < PRODUCT_IMAGE_MIN_HEIGHT) {
            if (PRODUCT_IMAGE_AUTO_ENHANCE_LOW_QUALITY) {
                try {
                    ...
                    console.warn('[product-image] low-quality enhance failed, continuing with original buffer:', String(error?.message || error));
                }
            }
            else if (PRODUCT_IMAGE_QUALITY_GATE_STRICT) {
                throw new Error(`Image is too small (${dimensions.width}x${dimensions.height}), min ${PRODUCT_IMAGE_MIN_WIDTH}x${PRODUCT_IMAGE_MIN_HEIGHT}`);
            }
            else {
                console.warn('[product-image] low-quality image accepted (strict gate disabled):', `${dimensions.width}x${dimensions.height} < ${PRODUCT_IMAGE_MIN_WIDTH}x${PRODUCT_IMAGE_MIN_HEIGHT}`);
            }
        }
    }
    try {
        ...
        try {
            ...
            throw new Error(`white background processing failed: ${String(fallbackError?.message || fallbackError || error?.message || error)}`);
        }
    }
    if (PRODUCT_IMAGE_SMART_ENHANCE_ENABLED) {
        try {
            ...
            console.warn('[product-image] smart enhance failed, continuing:', String(error?.message || error));
        }
    }
    return persistProcessedImage(buffer, filename, filePrefix);
};
exports.processQueuedProductImage = processQueuedProductImage;
