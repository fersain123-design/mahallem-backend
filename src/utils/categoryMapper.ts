...
export type BarcodeCategoryMappingResult = {
  category: string;
  confidence: number;
...
export type BarcodeCategoryMappingInput = {
  product_name?: string;
  generic_name?: string;
  brands?: string;
  categories?: string;
...
type MatchContext = {
  text: string;
  weight: number;
  source: 'name' | 'generic_name' | 'brands' | 'categories' | 'category_tags' | 'ingredients_text' | 'quantity' | 'barcode';
};

export const mapBarcodeProductToMahallemCategory = (
  product: BarcodeCategoryMappingInput
): BarcodeCategoryMappingResult => {
  ...
  for (const rule of CATEGORY_RULES) {
    ...
    for (const keywordRaw of rule.keywords) {
      ...
      for (const context of contexts) {
        ...
        if (!context.text.includes(keyword)) continue;
