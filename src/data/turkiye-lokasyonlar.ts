...
export const TURKIYE_LOKASYONLAR: Il[] = [
  {
    id: 'adana',
    name: 'Adana',
    plaka: '01',
...
export const IL_PLAKA_MAP: Record<string, string> = {};
TURKIYE_LOKASYONLAR.forEach(il => {
  IL_PLAKA_MAP[il.plaka] = il.id;
});

// Yardımcı fonksiyonlar
export const getIller = () => {
  return TURKIYE_LOKASYONLAR.map(il => ({
    id: il.id,
    name: il.name,
    plaka: il.plaka
  }));
...
export const getIlceler = (ilId: string) => {
  const il = TURKIYE_LOKASYONLAR.find(i => i.id === ilId);
  if (!il) return [];
...
export const getMahalleler = (ilId: string, ilceId: string) => {
  const il = TURKIYE_LOKASYONLAR.find(i => i.id === ilId);
  if (!il) return [];
