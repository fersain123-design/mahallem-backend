...
export type ProductProcessingJobPayload = {
  productId: string;
  vendorId: string;
  imageJobs: QueuedProductImageInput[];
};
...
const PRODUCT_PROCESSING_JOB_TIMEOUT_MS = Math.max(
  15000,
  Number(process.env.PRODUCT_PROCESSING_JOB_TIMEOUT_MS || 120000)
);

let redisConnection: IORedis | null = null;
let processingQueue: Queue<ProductProcessingJobPayload> | null = null;
let processingWorker: Worker<ProductProcessingJobPayload> | null = null;
let queueDegradedToFallback = false;
let queueDegradedLogged = false;
const fallbackInFlightJobs = new Set<string>();
...
const withJobTimeout = async (payload: ProductProcessingJobPayload): Promise<void> => {
  await Promise.race([
...
const getQueue = async () => {
  ...
  if (!processingQueue) {
    ...
    processingQueue = new Queue<ProductProcessingJobPayload>(QUEUE_NAME, {
      connection,
      defaultJobOptions,
    });
  }
...
const processJob = async (payload: ProductProcessingJobPayload): Promise<void> => {
  ...
  for (let idx = 0; idx < payload.imageJobs.length; idx += 1) {
    ...
    const url = await processQueuedProductImage(imageJob, `${payload.vendorId}_${payload.productId}_${idx + 1}`);
    urls.push(url);
  }
...
const processJobWithStatus = async (payload: ProductProcessingJobPayload): Promise<void> => {
  try {
...
const runFallbackJob = (payload: ProductProcessingJobPayload): string => {
...
export const startProductProcessingWorker = async (): Promise<void> => {
  ...
  try {
    ...
    processingWorker = new Worker<ProductProcessingJobPayload>(
      QUEUE_NAME,
      async (job: { data: ProductProcessingJobPayload }) => {
    ...
    processingWorker.on('completed', (job: { id?: string | number }) => {
      console.log(`[queue] product processing completed: ${job.id}`);
    });

    processingWorker.on('failed', (job: { id?: string | number } | undefined, err: unknown) => {
...
export const enqueueProductProcessingJob = async (payload: ProductProcessingJobPayload): Promise<string> => {
  if (!QUEUE_ENABLED) {
    if (!QUEUE_FALLBACK_ENABLED) {
      throw new Error('Product processing queue is disabled');
    }
...
export const isProductProcessingQueueEnabled = () => QUEUE_ENABLED || QUEUE_FALLBACK_ENABLED;
