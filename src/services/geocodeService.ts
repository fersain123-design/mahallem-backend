...
export interface GeocodedLocation {
  ...
  il: string;
  postalCode?: string;
  source: 'nominatim' | 'polygon' | 'cache';
}
...
interface NominatimAddress {
  ...
  municipality?: string;
  district?: string;
  // Additional fields Nominatim may return
...
const cache = new Map<string, { result: GeocodedLocation | null; ts: number }>();
...
function getCached(lat: number, lng: number): GeocodedLocation | null | undefined {
...
function setCache(lat: number, lng: number, result: GeocodedLocation | null): void {
 ...
 */
function extractMahalle(addr: NominatimAddress): string {
  ...
  for (const c of candidates) {
    ...
    if (notMahalle.has(key)) continue;

    // Skip if it looks like a district or city name rather than a neighborhood
    if (/^(ilçe|il|şehir|kent)\s*$/i.test(trimmed)) continue;
 ...
 */
function extractIlce(addr: NominatimAddress): string {
  ...
  for (const c of candidates) {
    ...
    if (isProvince(trimmed)) continue;
    return trimmed;
  }
 ...
 */
function extractIl(addr: NominatimAddress): string {
  const candidates = [
    addr.province,
    addr.state,
    addr.city,
  ...
  for (const c of candidates) {
    const trimmed = c.trim();
    if (!trimmed) continue;
    if (isProvince(trimmed)) return trimmed;
  }
...
export async function reverseGeocodeNominatim(
  ...
  if (cached !== undefined) {
    if (cached) console.log(`[GeocodeSvc] Cache hit: ${cached.mahalle}, ${cached.ilce}, ${cached.il}`);
    return cached;
  }
