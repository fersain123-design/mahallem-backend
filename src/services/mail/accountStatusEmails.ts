import { buildMailTemplate, PRIMARY_BUTTON_STYLE } from '../../utils/mailTemplate';
...
const getPortalButton = (role: RoleType) => {
  const href =
    role === 'VENDOR'
  ...
  return `<a href="${escapeHtml(href)}" style="${PRIMARY_BUTTON_STYLE}" target="_blank" rel="noopener noreferrer">${text}</a>`;
};

const buildSuspensionHtml = ({ name, role, reason, shopName }: SuspensionEmailInput) => {
  ...
  const optionalShop =
    role === 'VENDOR' && String(shopName || '').trim().length > 0
...
const buildUnsuspensionHtml = ({ name, role, shopName }: UnsuspensionEmailInput) => {
  const displayName = escapeHtml(formatDisplayName(name));
  const accountLabel = role === 'VENDOR' ? 'satıcı hesabınız' : 'müşteri hesabınız';
  const optionalShop =
    role === 'VENDOR' && String(shopName || '').trim().length > 0
...
export const sendSuspensionEmail = async (input: SuspensionEmailInput) => {
...
export const sendUnsuspensionEmail = async (input: UnsuspensionEmailInput) => {
  const to = String(input.to || '').trim().toLowerCase();
  if (!to) return;
