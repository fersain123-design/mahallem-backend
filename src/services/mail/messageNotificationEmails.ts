...
type SellerAdminSupportMessageEmailInput = {
  to: string;
  sellerName?: string | null;
  messageText?: string | null;
  hasImage?: boolean;
...
export const sendSellerNewMessageEmail = async (input: SellerMessageEmailInput) => {
  ...
  const content = `
    ...
    <div style="margin:0 0 16px 0;">
      <a href="${escapeHtml(panelUrl)}" style="${PRIMARY_BUTTON_STYLE}" target="_blank" rel="noopener noreferrer">Mesajı Görüntüle</a>
    </div>
...
export const sendSellerAdminSupportMessageEmail = async (input: SellerAdminSupportMessageEmailInput) => {
  const to = String(input.to || '').trim().toLowerCase();
  if (!to) return;
