...
const TMP_DIR = path.join(DATA_DIR, '_neighborhoods_tr_tmp');
...
const MAX_BACKOFF_MS = Number(process.env.OVERPASS_MAX_BACKOFF_MS ?? String(15 * 60_000)); // 15 minutes
...
const OVERPASS_FETCH_TIMEOUT_MS = Number(
  process.env.OVERPASS_FETCH_TIMEOUT_MS ?? String(OVERPASS_QL_TIMEOUT_SEC * 1000 + 60_000)
);

const OVERPASS_USER_AGENT = String(
  process.env.OVERPASS_USER_AGENT ?? 'MahallemDownloader/1.0 (+local dev)'
);
...
type Province = { name: string; osmId: number };
 ...
 */
const MVP_WHITELIST_NAMES = new Set<string>(['istanbul', 'ankara', 'izmir', 'adana', 'mersin', 'mugla', 'muğla']);
...
function normalizeProvinceNameForWhitelist(name: string): string {
  return String(name || '')
    .trim()
...
function buildTurkeyProvincesQuery(): string {
  return `
...
function buildRelationTagsQuery(ids: number[]): string {
...
interface ProvinceOutputFile {
  province: { name: string; osmId: number };
  createdAt: string;
...
function provinceOutFile(provinceOsmId: number): string {
  return path.join(TMP_DIR, `province-${provinceOsmId}.json`);
}

function isValidProvinceCheckpoint(filePath: string): boolean {
  const data = readJsonIfExists<ProvinceOutputFile>(filePath);
 ...
 */
function buildDistrictsQuery(provinceOsmId: number): string {
  return `
 ...
 */
function buildNeighborhoodsQuery(provinceOsmId: number): string {
  const adminLevelFilter = TR_MAHALLE_ADMIN_LEVELS.map(
    (lvl) =>
      `relation["admin_level"="${lvl}"]["boundary"="administrative"](area.province);
  way["admin_level"="${lvl}"]["boundary"="administrative"]["name"](area.province);
  way["admin_level"="${lvl}"]["boundary"="administrative"]["name:tr"](area.province);`
  ).join('\n  ');
...
function buildNeighborhoodsIndexQuery(provinceOsmId: number): string {
  const adminLevelFilter = TR_MAHALLE_ADMIN_LEVELS.map(
    (lvl) =>
      `relation["admin_level"="${lvl}"]["boundary"="administrative"](area.province);
  way["admin_level"="${lvl}"]["boundary"="administrative"]["name"](area.province);
...
function buildGeometryByIdsQuery(items: Array<{ type: 'relation' | 'way'; id: number }>): string {
 ...
 */
async function fetchOverpass(query: string, retries = MAX_RETRIES): Promise<OverpassElement[]> {
...
function extractDistrict(element: OverpassElement): string {
  const tags = element.tags || {};
  // Sometimes the is_in:district or is_in tag contains the district
  if (tags['is_in:district']) return tags['is_in:district'];
  if (tags['is_in:county']) return tags['is_in:county'];
  // addr:district
  if (tags['addr:district']) return tags['addr:district'];
  return '';
}

function getElementName(tags: Record<string, string> | undefined): string {
  const t = tags || {};
...
function dedupeConsecutive(points: number[][]): number[][] {
  ...
  for (const p of points) {
    ...
    if (prev && Math.abs(prev[0] - lng) < 1e-12 && Math.abs(prev[1] - lat) < 1e-12) continue;
...
function simplifyRDP(points: number[][], epsilon: number): number[][] {
  ...
  const distSqPointToSegment = (p: number[], a: number[], b: number[]) => {
    const px = p[0], py = p[1];
    const ax = a[0], ay = a[1];
    const bx = b[0], by = b[1];
    const dx = bx - ax;
    const dy = by - ay;
    if (Math.abs(dx) < 1e-20 && Math.abs(dy) < 1e-20) return sq(px - ax) + sq(py - ay);
...
function simplifyPolygonCoords(coords: number[][][], epsilon: number): number[][][] | null {
...
function simplifyMultiPolygonCoords(coords: number[][][][], epsilon: number): number[][][][] | null {
  if (!Array.isArray(coords) || coords.length === 0) return null;
  const out: number[][][][] = [];
  for (const poly of coords) {
    const simplified = simplifyPolygonCoords(poly as any, epsilon);
    if (simplified) out.push(simplified);
  }
  return out.length ? out : null;
}
...
function normalizeGeometry(
  ...
  epsilon: number
): { type: 'Polygon' | 'MultiPolygon'; coordinates: any } | null {
  if (geometry.type === 'Polygon') {
    const simplified = simplifyPolygonCoords(geometry.coordinates as number[][][], epsilon);
    if (!simplified) return null;
    return { type: 'Polygon', coordinates: simplified };
  }

  if (geometry.type === 'MultiPolygon') {
    const simplified = simplifyMultiPolygonCoords(geometry.coordinates as number[][][][], epsilon);
    if (!simplified) return null;
...
function computeBBoxOfRing(ring: number[][]): [number, number, number, number] | null {
...
function computeBBoxOfGeometry(geom: GeoJSONFeature['geometry']): [number, number, number, number] | null {
  let minLng = Infinity, minLat = Infinity, maxLng = -Infinity, maxLat = -Infinity;

  const visitPoint = (p: number[]) => {
    const lng = Number(p?.[0]);
    const lat = Number(p?.[1]);
  ...
  if (geom.type === 'Polygon') {
    ...
    for (const ring of coords) {
      for (const p of ring) visitPoint(p);
    }
  } else {
    const coords = geom.coordinates as number[][][][];
    for (const poly of coords) {
...
function bboxContains(b: [number, number, number, number], lat: number, lng: number): boolean {
...
function pointInGeometry(lat: number, lng: number, geom: GeoJSONFeature['geometry']): boolean {
  if (geom.type === 'Polygon') return pointInPolygon(lat, lng, geom.coordinates as number[][][]);
  return pointInMultiPolygon(lat, lng, geom.coordinates as number[][][][]);
}
...
function representativePointFromCoordinates(
  coordinates: number[][][] | number[][][][]
): { lat: number; lng: number } | null {
  const maybeMulti = Array.isArray(coordinates[0]) &&
    Array.isArray((coordinates as any)[0][0]) &&
    Array.isArray((coordinates as any)[0][0][0]);

  // Pick the largest polygon (by abs signed area of its outer ring).
  let bestOuter: number[][] | null = null;
  let bestAbsArea = 0;

  const considerOuter = (ring: number[][] | undefined | null) => {
  ...
  if (maybeMulti) {
    ...
    considerOuter((coordinates as number[][][])[0]);
  }
  ...
  if (c && Number.isFinite(c.lat) && Number.isFinite(c.lng)) {
    ...
    const inside = maybeMulti
      ? pointInMultiPolygon(c.lat, c.lng, coordinates as number[][][][])
      : pointInPolygon(c.lat, c.lng, coordinates as number[][][]);
    if (inside) return { lat: c.lat, lng: c.lng };
  }
 ...
 */
function elementToCoordinates(element: OverpassElement): number[][][] | number[][][][] | null {
  ...
  if (element.type === 'relation' && element.members) {
    ...
    if (mergedOuters.length === 0) return null;

    if (mergedOuters.length === 1) {
      // Single polygon (possibly with holes)
      const coords: number[][][] = [mergedOuters[0]];
    ...
    // MultiPolygon
    const multiCoords: number[][][][] = mergedOuters.map((outer) => [outer]);
 ...
 */
function mergeRings(rings: number[][][]): number[][][] {
  ...
  for (let i = 0; i < rings.length; i++) {
    ...
    if (merged.length >= 3) {
      ...
      const nearClosed =
        Math.abs(f[0] - l[0]) + Math.abs(f[1] - l[1]) < 1e-6;
...
async function getActiveProvinces(): Promise<Province[]> {
...
async function main() {
  ...
  const progressExisting = readJsonIfExists<ProgressFile>(PROGRESS_FILE);
  const progress: ProgressFile = progressExisting && progressExisting.version === 1
    ? progressExisting
    : {
        version: 1,
  ...
  progress.completed = (progress.completed || []).filter((c) => {
    if (!c || !Number.isFinite(c.osmId) || !c.file) return false;
    if (!fs.existsSync(c.file)) return false;
    if (!isValidProvinceCheckpoint(c.file)) return false;
    return Number(c.featureCount || 0) > 0;
  });

  const completedSet = new Set<number>(progress.completed.map((c) => c.osmId));
  const provinceFilesPresent = new Set<number>();
  for (const p of activeProvinces) {
    const f = provinceOutFile(p.osmId);
    if (fs.existsSync(f)) provinceFilesPresent.add(p.osmId);
  }

  // If file exists but progress is missing entry, backfill it.
  for (const p of activeProvinces) {
    if (completedSet.has(p.osmId)) continue;
    if (!provinceFilesPresent.has(p.osmId)) continue;
    ...
    const data = readJsonIfExists<ProvinceOutputFile>(filePath);
    if (!data || !Array.isArray(data.features) || (data.featureCount ?? data.features.length) <= 0) continue;
  ...
  const activeIdSet = new Set<number>(activeProvinces.map((p) => p.osmId));
  ...
  if (RESUME && progress.completed.length) {
    ...
    console.log(
      `â†» Resume detected: ${completedInActive.length}/${activeProvinces.length} active provinces already completed (${totalFeatureCount} features).`
    );
    console.log('');
  }
  ...
  for (let i = 0; i < activeProvinces.length; i++) {
    ...
    if (RESUME && completedSet.has(province.osmId) && fs.existsSync(provinceFile) && isValidProvinceCheckpoint(provinceFile)) {
      console.log(`${progressLabel} â­ï¸  ${province.name} (already downloaded)`);
      continue;
    }

    if (RESUME && fs.existsSync(provinceFile) && !isValidProvinceCheckpoint(provinceFile)) {
      try {
    ...
    for (let provinceAttempt = 1; provinceAttempt <= PROVINCE_MAX_RETRIES; provinceAttempt++) {
      ...
      try {
        ...
        const districtFeatures: Array<{ name: string; geom: GeoJSONFeature['geometry']; bbox: [number, number, number, number] }> = [];
        ...
        for (const el of districtElements) {
          ...
          const rawGeom: GeoJSONFeature['geometry'] = isMulti
        ...
        const centerByKey = new Map<string, { lat: number; lon: number }>();
        ...
        const indexItems = indexElements
          ...
          .map((el) => ({ type: el.type as 'relation' | 'way', id: el.id }));
        ...
        for (const el of elements) {
          if (!el || (el.type !== 'relation' && el.type !== 'way')) continue;
          if (el.center && Number.isFinite(el.center.lat) && Number.isFinite(el.center.lon)) continue;
        ...
        for (const el of elements) {
          ...
          if (Number.isFinite(centerLat) && Number.isFinite(centerLng) && districtFeatures.length > 0) {
            for (const d of districtFeatures) {
              if (!bboxContains(d.bbox, centerLat, centerLng)) continue;
        ...
        const provinceOut: ProvinceOutputFile = {
        ...
        if (existingIdx >= 0) progress.completed[existingIdx] = entry;
    ...
    if (lastErr) {
      const msg = (lastErr as any)?.message ? String((lastErr as any).message) : String(lastErr);
  ...
  for (const p of activeProvinces) {
    ...
    const data = readJsonIfExists<ProvinceOutputFile>(f);
